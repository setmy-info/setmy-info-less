...
- E2E tests now run against the Selenium Grid on **selenium.gintra** (`SELENIUM_HUB_URL` default);
  ...
  loopback — so `scripts/pageHelper.cjs` serves on all interfaces and builds the URL from this
  machine's address on the route toward the hub. `E2E_PAGE_HOST` overrides the host and
  `E2E_PAGE_PORT` pins the port.
