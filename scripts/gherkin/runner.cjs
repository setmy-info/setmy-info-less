/*
Executes Gherkin feature DTOs (see dto.cjs) as Jest e2e tests against pageHelper.
One Scenario = one Jest test; steps run sequentially in DTO order.
*/

const pageHelper = require("../pageHelper.cjs");
...
const ACTIONS = {
    ...
    pageElementMarginShouldBe: (margin) =>
        expect(pageHelper.data.computedStyles.margin).toBe(margin),
    pageElementPaddingShouldBe: (padding) =>
        expect(pageHelper.data.computedStyles.padding).toBe(padding),
    pageElementFontFamilyShouldBe: (fontFamily) =>
        expect(pageHelper.data.computedStyles.fontFamily).toBe(fontFamily),
    pageElementFontSizeShouldBe: (fontSize) =>
        expect(pageHelper.data.computedStyles.fontSize).toBe(fontSize),
    pageElementXShouldBe: (x) =>
        expect(pageHelper.data.computedStyles.x).toBe(x),
    pageElementYShouldBe: (y) =>
        expect(pageHelper.data.computedStyles.y).toBe(y),
    pageElementWidthShouldBe: (width) =>
        expect(pageHelper.data.computedStyles.width).toBe(width),
    pageElementHeightShouldBe: (height) =>
        expect(pageHelper.data.computedStyles.height).toBe(height),
    pageElementTopShouldBe: (top) =>
        expect(pageHelper.data.computedStyles.top).toBe(top),
    pageElementLeftShouldBe: (left) =>
        expect(pageHelper.data.computedStyles.left).toBe(left),
    pageElementBackgroundColorShouldBe: (color) =>
        expect(pageHelper.data.computedStyles.backgroundColor).toBe(color),
    pageElementColorShouldBe: (color) =>
        expect(pageHelper.data.computedStyles.color).toBe(color),
    pageElementStyleShouldBe: (propertyName, value) =>
        expect(pageHelper.data.computedStyles.allStyles[propertyName]).toBe(
            value,
        ),
